...
このリポジトリには、`.codex/` に native Codex subagent adapter も含まれています。これはカスタム runtime ではありません。Markdown roles を Codex custom agent files に対応させ、明示的に要求された場合、または standing user instruction で有効化された場合に Codex が実際の subagent threads を使えるようにします。
...
native Codex subagents が要求された場合、この protocol は `agent-team-protocol/10-native-subagents.md` と `.codex/agents/*.toml` を使い、roles を実際の Codex subagent threads に dispatch します。Lead は引き続き routing、approval、gates、loops、final release status を所有します。
...
Agent Team は 2 つの実行モードをサポートします：

```text
Simulated mode
  1 つの Codex conversation が Role Packets と artifacts によって role boundaries をシミュレートします。

Native Codex subagent mode
  Lead が選択した roles を独立した Codex subagent threads に dispatch し、artifacts を受け取ります。
```
...
native adapter には次の custom agents が含まれています：
...
Lead はデフォルトでは native subagent ではありません。Lead は parent orchestrator のままです。

Native subagents は、独立した探索、architecture checks、QA、code review、security review、context pollution が品質に影響する大きめのタスクに向いています。Implementation subagents はファイルを書けますが、明示的な user approval があり、ownership boundaries が明確な場合に限ります。
...
.codex/
  Native Codex subagent adapter。project-scoped custom agent files を含みます。
